...
fn try_zeroed(len: usize) -> Result<Zeroizing<Vec<u8>>, errors::ScryptError> {
    let mut buffer = Zeroizing::new(Vec::new());
    buffer
        .try_reserve_exact(len)
        .map_err(|_| errors::ScryptError::AllocationFailed)?;
    // `try_reserve_exact` above made this resize allocation-free. The wrapper
    // clears length and capacity on every normal/error return.
    buffer.resize(len, 0);
    Ok(buffer)
}

fn work_lengths(params: &Params) -> Result<(usize, usize, usize, usize), errors::ScryptError> {
    let n = 1usize
        .checked_shl(u32::from(params.log_n))
        .ok_or(errors::ScryptError::AllocationFailed)?;
    let r128 = (params.r as usize)
        .checked_mul(128)
        .ok_or(errors::ScryptError::AllocationFailed)?;
    let pr128 = (params.p as usize)
        .checked_mul(r128)
        .ok_or(errors::ScryptError::AllocationFailed)?;
    let nr128 = n
        .checked_mul(r128)
        .ok_or(errors::ScryptError::AllocationFailed)?;
    Ok((n, r128, pr128, nr128))
}

/// Total number of temporary bytes required by [`scrypt_fallible`].
///
/// This is exposed so callers can test and audit hostile parameter boundaries
/// without actually committing the corresponding amount of memory.
pub fn scrypt_work_bytes(params: &Params) -> Result<usize, errors::ScryptError> {
    let (_, r128, pr128, nr128) = work_lengths(params)?;
    pr128
        .checked_add(nr128)
        .and_then(|value| value.checked_add(r128))
        .ok_or(errors::ScryptError::AllocationFailed)
}

/// Allocation-aware scrypt with securely cleared temporary work buffers.
///
/// Unlike [`scrypt`], allocation failure is returned to the caller instead of
/// going through an infallible `vec![]` allocation. `B`, `V`, and `T` are all
/// wrapped in [`Zeroizing`] before they can contain password-derived state.
pub fn scrypt_fallible(
    password: &[u8],
    salt: &[u8],
    params: &Params,
    output: &mut [u8],
) -> Result<(), errors::ScryptError> {
    if output.is_empty() || output.len() / 32 > 0xffff_ffff {
        return Err(errors::ScryptError::InvalidOutputLen);
    }

    let (n, r128, pr128, nr128) = work_lengths(params)?;
    // Allocate everything before deriving the first password-dependent byte.
    let mut b = try_zeroed(pr128)?;
    let mut v = try_zeroed(nr128)?;
    let mut t = try_zeroed(r128)?;
    pbkdf2_hmac::<Sha256>(password, salt, 1, &mut b);
    for chunk in b.chunks_mut(r128) {
        romix::scrypt_ro_mix(chunk, &mut v, &mut t, n);
    }
    pbkdf2_hmac::<Sha256>(password, b.as_slice(), 1, output);
    Ok(())
}
