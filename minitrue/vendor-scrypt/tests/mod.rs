...
use scrypt::{Params, scrypt, scrypt_fallible};
...
#[test]
fn test_scrypt_fallible_known_vector() {
    // O vetor mínimo do RFC 7914 exercita diretamente a API local: não basta
    // manter verdes os vetores da função upstream que ainda aloca de forma
    // infalível.
    let t = &tests()[0];
    let params = Params::new(t.log_n, t.r, t.p).unwrap();
    let mut result = vec![0u8; t.expected.len()];
